...
CAMEやScheduleFreeなら1000-1500ステップがいい感じ。

## SD1/2が苦手な物

図形のような正確な描画を求められるもの(ヘイロー、文字など)。武器などの細長い手持ちの物体。

これらはSDXLで改善している。パラメータ数0.9BのSD1ごときには無理。
...
多分過学習。LR、ステップ数、Dim(Rank)のいずれかを下げると改善するかも。

しかし、データセットの相性が悪いと何をしても品質を改善できないこともある。
...
なお、(ほとんどの人には関係ないと思うが)APIアクセスの回数は1秒間に10回までとなる。正当な理由で1秒間に10回超のアクセスをしたければ、DiscordでGelbooru運営に連絡する。上限を引き上げてくれるかも。
